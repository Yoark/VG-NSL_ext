...
if __name__ == '__main__':
    ...
    # ! changed word_dim and embed_size from 512 to 768
    ...
    parser.add_argument('--request_bert_multi_layers', type=int, default=0)
    parser.add_argument('--init_embeddings_type', choices=['override', 'partial', 'partial-fixed', 'bert'], default='override')
    ...
    # TODO: 1. matching vocabulary in the pregiven set to bert vocabulary
    # TODO: 2. add new vocabulary to bert and perform pre-train
    vocab_pre = pickle.load(open(os.path.join(opt.data_path, 'vocab.pkl'), 'rb'))
    if opt.init_embeddings_type == 'bert':
        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
#        , \#additional_special_tokens=list(vocab_pre.word2idx.keys()))
#        import ipdb; ipdb.set_trace() 
        vocab = Vocabulary()
        vocab.word2idx = tokenizer.vocab
        from collections import OrderedDict
        vocab.idx2word = OrderedDict([(t[1], t[0]) for t in tokenizer.vocab.items()])
    else:
        vocab = vocab_pre
    opt.vocab_size = len(vocab)


    #! change vocab to bert vocab
    ...
    # Load data loaders
    train_loader, val_loader = data.get_train_loaders(opt,
