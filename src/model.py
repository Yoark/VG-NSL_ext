...
from utils import make_embeddings, l2norm, cosine_sim, sequence_mask, \
    index_mask, index_one_hot_ellipsis, make_embeddings_bert
import utils
...
class EncoderText(nn.Module):
    ...
    def __init__(self, opt, vocab_size, semantics_dim):
        ...
        self.embedding_type = opt.init_embeddings_type
        self.vocab_size = vocab_size
        self.semantics_dim = semantics_dim
        #! change this to bert_embeddings
        #TODO use bert embedding in forward pass of VGNSL, also in this class 
        #self.sem_embedding = make_embeddings_bert(opt, self.semantics_dim)
        if self.embedding_type == 'bert':
            self.sem_embedding = make_embeddings_bert(opt, self.vocab_size, self.semantics_dim)
        else:
    ...
    def forward(self, x, lengths, volatile=False, target_align=None, length_whole=None):  
        ...
        if self.embedding_type == 'bert':
            ...
            sem_embeddings = sem_embeddings/sem_embeddings.max(dim=2, keepdim=True)[0]
            # print("bert go!")
        else:
...
class VGNSL(object):
    ...
    def forward_emb(self, images, captions, lengths, target_align=None, length_whole=None, volatile=False):
        ...
        if torch.cuda.is_available():
            ...
            target_align = target_align.cuda()
        with torch.set_grad_enabled(not volatile):
            img_emb = self.img_enc(images)
            txt_outputs = self.txt_enc(captions, lengths, volatile, target_align, length_whole)
        return (img_emb, ) + txt_outputs

    def forward_reward(self, base_img_emb, cap_span_features, left_span_features, right_span_features,
                       word_embs, lengths, span_bounds, whole_length, **kwargs):
        """Compute the loss given pairs of image and caption embeddings
        """
        lengths = whole_length
    ...
    def train_emb(self, images, captions, lengths, ids=None, target_align=None, lengths_whole=None, epoch=None, *args):
