...
from utils import generate_tree, clean_tree, generate_tree_bert
...
def encode_data(model, data_loader, log_step=10, logging=print, vocab=None, stage='dev'):
    ...
    cap_embs = None
    logged = False
    if model.opt.init_embeddings_type == 'bert':
        gen_tree = generate_tree_bert
    else:
    ...
    for i, (images, captions, lengths, ids) in enumerate(data_loader):
        ...
        if (not logged) or (stage == 'test'):
            ...
            for j in range(sample_num):
                logging(gen_tree(model.opt, captions, tree_indices, j, vocab))
...
def test_trees(model_path):
    ...
    data_loader = get_eval_loader(
        opt, opt.data_path, 'test', vocab, opt.batch_size, opt.workers, 
    ...
    for i, (images, captions, lengths, ids) in enumerate(data_loader):
        ...
        for j in range(len(ids)):
            candidate_trees.append(gen_tree(opt, captions, tree_indices, j, vocab))
